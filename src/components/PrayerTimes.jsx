...
const PrayerTimes = () => {
  ...
  const fetchTimes = async (lat, lng) => {
    ...
    try {
      ...
      if (data.code === 200) {
        ...
        STORAGE_SERVICE.setItem('athr_prayer_times', data.data.timings); // حفظ المواقيت للتفعيل التلقائي
  ...
  if (loading && !timings) return (
    <div className="p-8 bg-surface/90 rounded-3xl animate-pulse flex flex-col items-center justify-center gap-4 border border-black/5 shadow-[0_8px_32px_rgba(0,0,0,0.05)]">
  ...
  if (error && !timings) return (
    <div className="bg-surface/90 p-8 rounded-3xl border border-red-500/20 text-center shadow-[0_8px_32px_rgba(0,0,0,0.05)]">
  ...
  return (
    <div className="space-y-4">
      ...
      {nextPrayer && (
        <div className="bg-gradient-to-br from-green-main to-green-mid p-6 rounded-3xl border border-black/5 shadow-[0_8px_32px_rgba(0,0,0,0.1)] text-center relative overflow-hidden">
          ...
          <button
            ...
            className="mt-4 mx-auto flex items-center gap-2 bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl border border-white/20 transition-all font-bold text-white shadow-sm"
          >
            <span className="text-lg">🕌</span>
            <span className="text-sm font-tajawal">دخول وضع الصلاة</span>
          </button>

          <div className="mt-4 flex justify-center items-center gap-2 text-[10px] text-green-100/60">
            <span>📍 {location}</span>
      ...
      {/* Prayer Times Grid */}
      <div className="bg-surface/90 p-5 rounded-3xl border border-black/5 shadow-[0_8px_32px_rgba(0,0,0,0.05)]">
