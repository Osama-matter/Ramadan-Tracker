...
const RamadanPlanner = () => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-1000 pb-32 px-4 min-h-screen -mx-4 pt-8 bg-transparent transition-colors overflow-hidden">
      {/* Header */}
      <div className="text-center relative">
        ...
        <h2 className="text-4xl font-bold text-text-dark font-scheherazade relative">جدول الأثر اليومي</h2>
          ...
          {Array.from({ length: 30 }, (_, i) => i + 1).map((day) => (
            <button
              ...
              className={`flex-shrink-0 w-16 h-20 rounded-[1.8rem] flex flex-col items-center justify-center border-2 transition-all duration-500 ${currentDay === day
                ...
                : 'bg-surface border-black/5 text-text-mid shadow-sm'
                }`}
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {stats.filter(s => s.total > 0).map(s => (
          <div key={s.name} className="bg-surface p-5 rounded-[2rem] border border-black/5 shadow-sm relative">
            <div className="flex justify-between items-center mb-4">
              <span className="text-[11px] font-bold text-gold-dark">{s.name}</span>
      ...
      {planText && (
        <div className="bg-surface dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-[3rem] p-9 shadow-sm relative overflow-hidden">
          <h3 className="text-2xl font-bold text-green-main dark:text-gold font-scheherazade flex items-center gap-4 mb-6">
      ...
      <div className="space-y-8">
        <div className="flex justify-between items-center px-4">
          <div className="space-y-1">
            <h3 className="text-3xl font-bold text-text-dark font-scheherazade">مهام اليوم {currentDay.toLocaleString('ar-SA')}</h3>
        ...
        {showAddForm && (
          <form
            onSubmit={addTask}
            className="bg-surface border-2 border-gold/40 rounded-[3rem] p-10 shadow-2xl animate-in zoom-in-95 duration-500"
          >
            <input
              autoFocus
              value={newTaskText}
        ...
        <div className="grid gap-5 px-1">
          {tasks.length > 0 ? (
            tasks.map((task) => (
              <div
                key={task.id}
                className={`group flex items-center justify-between p-6 bg-surface border border-black/5 rounded-[2.5rem] transition-all duration-700 hover:border-gold/50 shadow-sm ${task.completed ? 'opacity-30' : ''
                  }`}
              >
                <div
            ...
            <div className="py-24 text-center space-y-8 bg-surface/40 border-2 border-dashed border-black/5 rounded-[4rem] opacity-30">
